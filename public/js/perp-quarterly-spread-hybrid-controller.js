/**
 * Perp-Quarterly Spread Hybrid Controller
 * 
 * DUAL API APPROACH:
...
function perpQuarterlySpreadHybridController() {
    return {
        // Global state
        ...
        // Initialize
        init() {
            console.log('🚀 Perp-Quarterly Spread Hybrid Dashboard initialized');
        ...
        exportChart(format = 'png') {
            ...
            try {
                ...
                const filename = `Perp_Quarterly_Spread_Chart_${this.selectedExchange}_${timestamp}`;
        ...
        shareChart() {
            ...
            try {
                ...
                const shareData = {
                    title: `Bitcoin Perp-Quarterly Spread - ${this.selectedExchange}`,
                    text: `Current Open Interest: ${this.formatOI(this.currentOI)} | Signal: ${this.marketSignal}`,
     ...
     // Load Bitcoin price data from CryptoQuant API only (NO DUMMY DATA)
        ...
        renderChart() {
            const canvas = document.getElementById('perpQuarterlySpreadMainChart');
            if (!canvas) {
                console.warn('⚠️ Canvas element not found: perpQuarterlySpreadMainChart');
                return;
            }
   ...
   // Render distribution chart (histogram) - ADAPTED FOR OPEN INTEREST
        renderDistributionChart() {
            const canvas = document.getElementById('perpQuarterlySpreadDistributionChart');
            if (!canvas) return;
        ...
        renderMAChart() {
            const canvas = document.getElementById('perpQuarterlySpreadMAChart');
            if (!canvas) return;
...
console.log('✅ Perp-Quarterly Spread Hybrid Controller loaded');

// Initialize when DOM is ready
document.addEventListener('DOMContentLoaded', function () {
    // Make sure Alpine.js can access the controller
    if (typeof window.Alpine !== 'undefined') {
        console.log('🔗 Registering Perp-Quarterly Spread Hybrid controller with Alpine.js');
    }
});

// Make controller available globally for Alpine.js
window.perpQuarterlySpreadHybridController = perpQuarterlySpreadHybridController;